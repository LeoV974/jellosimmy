    ...
    for (i = 0; i < xNodes; i++) {
        for (j = 0; j < yNodes; j++) {
            for (k = 0; k < zNodes; k++) {
                ...
                // X-axis springs
                if (i > 0) { // -x
                    linearSpring = new SIMMY.LinearSpring(node, nodesDict[(i-1)+"_"+j+"_"+k], diffX, kLinearSpring);
                ...
                // Y-axis springs
                if (j > 0) { //-y
                    linearSpring = new SIMMY.LinearSpring(node, nodesDict[i+"_"+(j-1)+"_"+k], diffY, kLinearSpring);
                    node.addSpring(linearSpring);
